...
async function cargar() {
    ...
    try{
        // cambio
        const response  = await fetch(`http://localhost:8000/api/personal/empleados/${empleado_id}`);
        console.log(`http://localhost:8000/api/personal/empleados/${empleado_id}`)
        ...
        marcarDiasSeleccionados(item.dia);
    }catch(error){
...
document.getElementById("cargar").addEventListener("click", async () => {
    ...
    const empleadoData = {
        ...
        const putResponse = await fetch(`http://localhost:8000/api/personal/empleados/${empleado_id}`, {
            method: 'PUT',
            headers: {
